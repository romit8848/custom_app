...
def get_party_tax_withholding_details(inv, tax_withholding_category=None):
	if inv.doctype == "Payment Entry": # Custom Change
...
def get_tax_amount(party_type, parties, inv, tax_details, posting_date, pan_no=None):
	...
	if party_type == "Supplier":
		...
		if tax_deducted:
			...
			net_total = inv.tax_withholding_net_total # custom change
			if ldc:
...
def get_tds_amount(ldc, parties, inv, tax_details, tax_deducted, vouchers):
	...
	# field = "sum(net_total)"
	field = 'sum(tax_withholding_net_total)' # custom change
	...
	supp_credit_amt += inv.tax_withholding_net_total # custom change
	...
	if (threshold and inv.tax_withholding_net_total >= threshold) or ( # custom change
		...
		if (cumulative_threshold and supp_credit_amt >= cumulative_threshold) and cint(
			...
			net_total = frappe.db.get_value("Purchase Invoice", invoice_filters, "sum(tax_withholding_net_total)") or 0.0 # custom change
			# net_total += inv.net_total
			net_total += inv.tax_withholding_net_total # custom change
		...
		if ldc and is_valid_certificate(
			...
			tax_deducted,
			inv.tax_withholding_net_total, # custom change
...
def get_tds_amount_from_ldc(ldc, parties, pan_no, tax_details, posting_date, net_total):
	...
	limit_consumed = frappe.db.get_value(
		...
		"sum(tax_withholding_net_total)", # custom change
	)
